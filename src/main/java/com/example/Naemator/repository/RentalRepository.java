...
public interface RentalRepository extends CrudRepository<Rental, Long> {
    ...
    public List<Rental> findAllByListingOwnerId(@Param("ownerId") Long ownerId);

    @Query(nativeQuery = true, value = "SELECT rental_id, end_date, start_date, total_price, listing_listing_id, renter_user_id, r.city, address FROM `rentals` r\n" +
            "WHERE r.renter_user_id = :renterId;")
    public List<Rental> findAllByRenterId(@Param("renterId") Long renterId);

    @Query(nativeQuery = true, value = "SELECT rental_id, end_date, start_date, total_price, listing_listing_id, renter_user_id, r.city, address FROM `rentals` r\n" +
            "WHERE r.listing_listing_id = :listingId;")
    public List<Rental> findAllByListingId(@Param("listingId") Long listingId);
}