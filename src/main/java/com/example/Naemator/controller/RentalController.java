...
public class RentalController {
    ...
    public String getMyRents(Principal principal, Model model) {
        ...
        List<Rental> rentals = rentalRepository.findAllByRenterId(renter.getId());
        model.addAttribute("rentals", rentals.reversed());
    ...
    public String getMyListingsRents(Principal principal, Model model) {
        ...
        List<Rental> rentals = rentalRepository.findAllByListingOwnerId(owner.getId());
        model.addAttribute("rentals", rentals.reversed());
        model.addAttribute("encoder", new ImageEncoder());
    ...
    @ResponseBody
    @GetMapping("/api/get-busy-dates-for-listing/{listingId}")
    public List<LocalDate> getBusyDatesForListing(@PathVariable("listingId") Long listingId) {
        return rentalService.getBusyDatesForListing(listingId);
    }
}